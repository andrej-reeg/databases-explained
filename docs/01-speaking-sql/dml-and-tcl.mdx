...
`INSERT` adds new rows. List the columns, then the values. Here we add a seventh customer (ids 1-6 are taken, so we use `7`):
...
Run this. The first count is `0` (rows gone inside the transaction); after `ROLLBACK` the second count is back to `5`:
