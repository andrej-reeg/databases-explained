...
- **Eventual consistency** - replicas may lag briefly but all converge to the same value once writes stop.
...
- **IVF (Inverted File index)** - an approximate-nearest-neighbor method that clusters vectors and searches only the nearest clusters.
...
- **Synchronous vs asynchronous replication** - synchronous waits for replicas to confirm a write (safer, slower); asynchronous does not (faster, can lose recent writes).
...
- **TrueTime** - Google Spanner's API that bounds clock uncertainty across datacenters, enabling globally consistent transaction ordering.
