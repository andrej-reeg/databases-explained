...
If the query only selects columns already in the index key - plus the `rowid` SQLite carries for free - the same `(status, created_at)` index becomes a **covering index**: it answers the query from the index alone, with no table fetch (the plan reads `USING COVERING INDEX`). To cover an extra column in SQLite you add it to the index key, e.g. `(status, created_at, id)`; SQLite has no separate `INCLUDE` clause (that is a Postgres/SQL-Server feature).
