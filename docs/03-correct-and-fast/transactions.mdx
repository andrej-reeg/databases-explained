...
- **Consistent lock ordering to kill deadlocks.** Reserve and claim touched two tables in *different* orders and deadlocked - exactly the [opposite-order recipe](#deadlocks-two-transactions-opposite-order) above. The fix was the textbook one: make every path acquire locks in the **same order** (reserve always `DELETE`s from the units table first, then `INSERT`s into `reserved_quantities`).
